...
import { Heart, ShoppingBag, Star, ChevronLeft, Truck, RotateCcw, Shield } from 'lucide-react'
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/" className="text-sm underline">Retour a l&apos;accueil</Link>
        </div>
      </div>
    )
  }
    ...
    <div>
      <div className="max-w-7xl mx-auto px-4 py-4 md:py-8">
        ...
        <div className="flex items-center gap-2 text-xs text-stone-500 mb-4 md:mb-6">
          ...
          <span>/</span>
          <Link href={`/category/${product.category}`} className="hover:text-stone-900 capitalize">{product.category}</Link>
          <span>/</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
          ...
          <div>
            <div className="relative bg-stone-50 rounded-lg overflow-hidden aspect-square mb-3">
              ...
              />
              {product.badge && (
                <span className={`absolute top-3 left-3 px-2 py-1 text-[10px] font-bold uppercase ${
                  product.badge === 'HIT' ? 'bg-stone-900 text-white' : 'bg-red-600 text-white'
                }`}>
                  {product.badge}
                </span>
              )}
          ...
          <div>
              ...
              <div className="flex">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star
                    key={i}
                    size={16}
                    className={i < Math.floor(product.rating) ? 'text-yellow-400 fill-yellow-400' : 'text-stone-300'}
                  />
                ))}
              </div>
            ...
            <div className="flex items-baseline gap-3 mt-4">
              <span className="text-2xl md:text-3xl font-bold text-stone-900">{product.price} TND</span>
              {product.original_price > product.price && (
                <>
                  <span className="text-lg text-stone-400 line-through">{product.original_price} TND</span>
                  <span className="text-sm bg-red-100 text-red-700 px-2 py-0.5 rounded font-medium">
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              <span className="text-xs text-stone-400">- Livraison gratuite des 50 TND</span>
            </div>
            ...
            <div className="mt-6 flex gap-3">
              <button
                ...
                className="flex-1 h-12 bg-stone-900 text-white font-semibold text-sm uppercase tracking-wider hover:bg-stone-800 transition-colors flex items-center justify-center gap-2"
              >
                <ShoppingBag size={18} />
                Ajouter au panier
              </button>
              <button
                onClick={() => {}}
                className="w-12 h-12 border border-stone-200 rounded flex items-center justify-center hover:border-stone-400 transition-colors"
            ...
            <button
              ...
              className="w-full h-12 mt-3 border-2 border-stone-900 text-stone-900 font-semibold text-sm uppercase tracking-wider hover:bg-stone-50 transition-colors"
            >
              Acheter maintenant
            </button>
            ...
            <div className="mt-6 grid grid-cols-3 gap-3 border-t border-stone-100 pt-6">
              <div className="flex flex-col items-center text-center gap-1.5">
                ...
                <span className="text-[11px] text-stone-600">Livraison rapide</span>
              </div>
              <div className="flex flex-col items-center text-center gap-1.5">
                <RotateCcw size={20} className="text-stone-600" />
                <span className="text-[11px] text-stone-600">Retours 30 jours</span>
              </div>
              <div className="flex flex-col items-center text-center gap-1.5">
                <Shield size={20} className="text-stone-600" />
                <span className="text-[11px] text-stone-600">Produits 100% authentiques</span>
              </div>
            </div>
