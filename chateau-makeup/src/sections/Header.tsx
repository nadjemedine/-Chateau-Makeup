  ...
  return (
    <>
      ...
      <div className="bg-stone-900 text-white text-center py-2 px-4 text-xs md:text-sm">
        <span>Livraison GRATUITE des 50 d&apos;achat | Retours gratuits sous 30 jours</span>
      </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between px-4 py-3 md:py-4">
            ...
            <Link href="/" className="flex flex-col items-center">
              <h1 className="text-2xl md:text-3xl font-bold tracking-[0.25em] uppercase text-stone-900">
                Chateau
              </h1>
              <span className="text-[0.6rem] md:text-[0.65rem] tracking-[0.4em] uppercase text-stone-500 -mt-1">
                Makeup
              </span>
            </Link>
            ...
            <div className="flex items-center gap-1 md:gap-2">
              ...
              <Link href="/admin" className="hidden md:flex p-2 hover:bg-stone-100 rounded-full transition-colors">
                <User size={20} />
              </Link>
            ...
            {categories.map((cat) => (
              <Link
                ...
                className={`text-xs lg:text-sm font-medium uppercase tracking-wider whitespace-nowrap transition-colors hover:text-stone-600 ${
                  isActive(`/category/${cat.slug}`) ? 'text-stone-900 border-b-2 border-stone-900 pb-0.5' : 'text-stone-600'
                }`}
              >
                {cat.name}
              </Link>
          ...
          <div className="md:hidden absolute top-full left-0 right-0 bg-white border-b border-stone-200 shadow-lg z-50 max-h-[70vh] overflow-y-auto">
            <div className="p-4">
              ...
              <div className="border-t border-stone-100 mt-4 pt-4 space-y-1">
                <Link href="/cart" className="flex items-center py-3 px-3 text-sm font-medium text-stone-700 hover:bg-stone-50 rounded-lg" onClick={() => setMobileMenuOpen(false)}>
                  <ShoppingBag size={18} className="mr-3" /> Mon Panier
                </Link>
                <Link href="/admin" className="flex items-center py-3 px-3 text-sm font-medium text-stone-700 hover:bg-stone-50 rounded-lg" onClick={() => setMobileMenuOpen(false)}>
                  <User size={18} className="mr-3" /> Administration
                </Link>
              </div>
            </div>
          </div>
