...
export function ProductCard({ product }: ProductCardProps) {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    toast({
      ...
      description: `${product.name} a ete ajoute`,
    })
  }
    ...
    <Link href={`/product/${product.id}`} className="group block">
      <div className="relative bg-stone-50 rounded-lg overflow-hidden aspect-[3/4]">
        ...
        <div className="absolute top-2 right-2 flex flex-col gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
          <button className="w-8 h-8 bg-white rounded-full shadow-md flex items-center justify-center hover:bg-stone-100" aria-label="Add to favorites">
            <Heart size={14} />
          </button>
          <button
            onClick={handleAddToCart}
            className="w-8 h-8 bg-white rounded-full shadow-md flex items-center justify-center hover:bg-stone-100"
      ...
      <div className="mt-2.5 px-0.5">
        ...
        <div className="flex items-center gap-2 mt-1.5">
          <span className="text-sm font-bold text-stone-900">{product.price} TND</span>
          {product.original_price > product.price && (
            <span className="text-xs text-stone-400 line-through">{product.original_price} TND</span>
          )}
        </div>
      </div>
    </Link>
